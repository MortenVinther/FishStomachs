...
Creation of data for bootstrapping is done by the function bootstrap_data(), which create a new STOMobs object including the bootstrap replicate data.
The result of the call, bd, is a list of STOMobs object.
...
The bootstrap mean and variance from replicates are calculated by a call to bootsMean(). 
First the raw observations are loaded.
```{r ex06_13}
...
With more replicates, loaded from package data set, simple mean and variance, and Dirichlet parameters are calculated. See help(bootsMean) for more information. 
