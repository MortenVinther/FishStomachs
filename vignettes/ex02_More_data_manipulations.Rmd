...
A series of data manipulations, with addition of new variables, are needed before the population diet (or food ration) can be calculated. This example shows how the FishStomachs function canbe used.


### Bias correct, energy contents, arament etc.

Bias correction of stomach evacuation rate and thereby stomach content due to different energy contents, armament of the prey items can optional be done by the function  \code{bias_correct_energy_etc}. The function call uses several extra information kept on files for the bias correction, see the help for bias_correct_energy_etc(). In this example, these input files a supplied with the FishStomachs. 
...
Function \code{make_length_classess} transform the specifications into size classes by individual year and quarters.
