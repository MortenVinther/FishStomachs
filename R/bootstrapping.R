...
plotboots.size<-function(b,show_plot=TRUE,cut_pred_size=c(1,10),cut_prey_size=c(1,10),addTitle=FALSE,tAngle=90,
  ...
  x<-x %>%
    ...
    dplyr::select(key,stratum_time,pred_name, pred_size,n_tot,prey_name,prey_size,prey_w,rep)


  out<-by(x,list(x$pred_name,x$pred_size,x$stratum_time),function(x) {
    if (addTitle) tit<- paste(as.character(x$stratum_time)[1] ,as.character(x$pred_name)[1],as.character(x$pred_size)[1]) else tit<-NULL
    a<-ggplot(x,aes(prey_w)) +
      ...
      geom_histogram(col=Colours,bins=as.integer(min(reps/20,25)))+
...
#' @importFrom rlang .data
#' @export
plotboots<-function(b,show_plot=TRUE,cut_pred_size=c(1,10),cut_prey_size=c(1,10),addTitle=FALSE,tAngle=90,
                         Colours='red') {
  key<-n_tot<-one<-pred_name<-pred_size<-prey_w<-quarter<-quarter<-year<-NULL
  control<-attr(b[[1]],'control')
  # to one data frame
  ...
  x<-x %>%
    dplyr::mutate(year=as.integer(eval(control@strata_year_back)),
                  ...
                  pred_size=substr(pred_size,cut_pred_size[1],cut_pred_size[2]),
                  prey_size=substr(prey_size,cut_prey_size[1],cut_prey_size[2])) %>%
                  dplyr::group_by(key,stratum_time,pred_name, pred_size,n_tot,prey_name,rep) %>%
