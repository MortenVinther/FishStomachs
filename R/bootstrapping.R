...
#' @export
#' @examples \dontrun{s<-add_missing(boots(s,value=NA))}
...
plotboots<-function(b,show_plot=TRUE,freq=TRUE,cut_pred_size=c(1,10),addTitle=FALSE,tAngle=90,
  ...
  out<-by(x,list(x$pred_name,x$stratum_time),function(x) {
    ...
    if (addStat)  {
     a<- a+geom_line(data = df2, aes(y=y), color = "#C10534", size = 1, alpha = 0.75)+
         geom_vline(aes(xintercept=mu), linetype="dotted",color = Colours[3], size=1.5,data = df)
    }
