...
#'
#' Size classes of both predators and preys must have been assigned ( \link{put_size_class_on_predator} and \link{put_size_class_on_prey}) aggregation can be made.
#' @title Aggregate stomach contents within sample
#' @param stom  stomach contents data of class STOMobs.
#' @return stomach contents data of class STOMobs with aggregated stomach contents and updated predator information.
#' @export
...
aggregate_within_sample <- function(stom) {
   ...
   pred<-stom[['PRED']] %>% dplyr::mutate(n_food=NULL)
    ...
    prey<-dplyr::left_join(stom[['PREY']],dplyr::select(pred,sample_id,fish_id,pred_size), by = c("sample_id", "fish_id"))
   ...
   nstom<-pred %>% dplyr::mutate(nl=n_tot*pred_l_mean)  %>% dplyr::group_by(pred_name,sample_id,pred_size_class,pred_size) %>%
    dplyr::summarise(n_tot=sum(n_tot),nl=sum(nl)) %>% dplyr::mutate(pred_l_mean=as.integer(nl/n_tot),nl=NULL) %>% dplyr::ungroup()
  ...
  # as.data.frame(dplyr::filter(prey,prey_l_mean>600 & prey_l_mean !=9999L))
  # as.data.frame(dplyr::filter(prey,prey_l_mean_num>600 & prey_l_mean !=9999L))
  # xtabs(~prey_l_mean,data=prey)
  ...
  prey<-prey %>% dplyr::mutate(prey_l_mean_num=dplyr::if_else(prey_l_mean==mis_l,as.integer(NA),prey_l_mean))   %>%
    ...
    dplyr::mutate(prey_l_mean=prey_l_mean/prey_n) %>% dplyr::rename(fish_id=pred_size)
