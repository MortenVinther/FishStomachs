...
#' @param stomach_consum bias correct stomach observations or estimate unbiased food consumption
...
#' @export
#'
bias_correct_energy_etc<-function(x,nodc_group_file,param_file,temperature_file,energy_file,hour_multiplier=1,pred_length_multiplier=1,stomach_consum=c('stom','cons')[1]) {
  ...
  stopifnot(stomach_consum %in% c('stom','cons'))
  ...
  prey<-dplyr::left_join(x=prey,y=en,by = c("quarter", "species_group", "l")) %>% dplyr::mutate(quarter=NULL,l=NULL)
...
PREY<<-prey
 if (stomach_consum==c('stom','cons')[2]) {
 ...
 if (stomach_consum==c('stom','cons')[1]) {
    prey<-prey %>% dplyr::mutate(prey_w=prey_w/evacRate) %>%
    dplyr::select(-pred_l, -n_tot, -n_food, -delta, -rho,-lambda,-xi, -alfa, -k, -Temp,-E, -Armer, -avg_E,-S,-b,-species_group,  -ration1,  -evacRate)
 }
